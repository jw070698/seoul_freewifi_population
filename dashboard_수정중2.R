  ...
  dashboardBody(
    tabItems(
      ...
      ,tabItem(tabName = "INSIGHT",
               fluidRow(h3(" - 년도별누적x wifi설치 현황과 생활인구 현황(참고자료) 비교"),
                column(width = 3,
                        box(radioButtons(inputId = "popwifi", # 누적x wifi랑 생활인구그래프
                                         "그래프를 고르세요",width=200,
                                         choices=c("2019년","2020년","2021년")))),
                column(width=9,
                        box(title="누적하지 않은 wifi설치 총개수",status="warning",solidHeader=TRUE,collapsible=TRUE,
                              imageOutput("nocum"),width = 100 ))), # 누적안한그래프
               fluidRow(width=10, offset=2,
