    ...
    tabItems(
      ...
      tabItem(tabName = "EDA",
              ...
              fluidRow(h3(" - 행정동별 서울생활인구(내국인)"),
                      ...
                      column(4,
                               box(status = "primary",solidHeader = TRUE, collapsible = TRUE, plotOutput(outputId = "pieChart", width = 400)))
              ),
...
server <- function(input, output,session) {
  ...
  output$pieChart <- renderPlot({
    ...
    pie(circle, labels=label, main = "서울시 내 자치구 총생활인구 top5")
  })
...
" tags$h1('빅데이터 통계분석 1조 대시보드'),
  tags$hr(),
  tags$br(),
  tags$p(strong('총생활인구수를 통한 와이파이 추가적인 설치 필요 지역 탐색')),
